...
def data_process(X_drug = None, X_target = None, y=None, drug_encoding=None, target_encoding=None, 
	...
	if DTI_flag:
		...
		if split_method == 'random': 
			...
			train, val, test = a(df_data, random_seed, frac)
...
class data_process_Multimodel_loader(data.Dataset):

	def __init__(self, list_IDs, labels, df, configlist):
		'Initialization'
		self.labels = labels
		self.list_IDs = list_IDs
		self.df = df
		...
		for i in range(len(configlist)):
			conf = self.config[i]
			if conf['drug_encoding'] in ['DGL_GCN', 'DGL_NeuralFP']:
				from dgllife.utils import smiles_to_bigraph, CanonicalAtomFeaturizer, CanonicalBondFeaturizer
				self.node_featurizer = []
				self.node_featurizer.append(CanonicalAtomFeaturizer())
				self.edge_featurizer = []
				self.edge_featurizer.append(CanonicalBondFeaturizer(self_loop = True))
				from functools import partial
				self.fc = partial(smiles_to_bigraph, add_self_loop=True)

			elif conf['drug_encoding'] == 'DGL_AttentiveFP':
				from dgllife.utils import smiles_to_bigraph, AttentiveFPAtomFeaturizer, AttentiveFPBondFeaturizer
				self.node_featurizer = []
				self.node_featurizer.append(AttentiveFPAtomFeaturizer())
				self.edge_featurizer = []
				self.edge_featurizer.append(AttentiveFPBondFeaturizer(self_loop=True))
				from functools import partial
				self.fc = partial(smiles_to_bigraph, add_self_loop=True)

			elif conf['drug_encoding'] in ['DGL_GIN_AttrMasking', 'DGL_GIN_ContextPred']:
				from dgllife.utils import smiles_to_bigraph, PretrainAtomFeaturizer, PretrainBondFeaturizer
				self.node_featurizer = []
				self.node_featurizer.append(PretrainAtomFeaturizer())
				self.edge_featurizer = []
				self.edge_featurizer.append(PretrainBondFeaturizer())
				from functools import partial
				self.fc = partial(smiles_to_bigraph, add_self_loop=True)
	def __len__(self):
		'Denotes the total number of samples'
	...
	def __getitem__(self, index):
		...
		for i in range(len(self.config)):
			v_d = self.df.iloc[index]['drug_encoding_{}'.format(i+1)]
			if self.config[i]['drug_encoding'] == 'CNN' or self.config[i]['drug_encoding'] == 'CNN_RNN':
				v_d = drug_2_embed(v_d)
			elif self.config[i]['drug_encoding'] in ['DGL_GCN', 'DGL_NeuralFP', 'DGL_GIN_AttrMasking', 'DGL_GIN_ContextPred', 'DGL_AttentiveFP']:
				v_d = self.fc(smiles = v_d, node_featurizer = self.node_featurizer, edge_featurizer = self.edge_featurizer)
			y = self.labels[index]
			v_ds.append(v_d)
		return v_ds, y
