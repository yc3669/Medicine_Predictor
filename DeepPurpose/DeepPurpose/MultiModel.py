...
class Classifier(nn.Sequential):
    def __init__(self, model_drug, configlist):
        ...
        self.hidden_dims = configlist[0]['cls_hidden_dims']
        layer_size = len(self.hidden_dims) + 1
        input_dim_drug = 0
        for i in range(len(configlist)):
            input_dim_drug = input_dim_drug + configlist[i]['hidden_dim_drug']
        dims = [input_dim_drug] + self.hidden_dims + [1]

        self.predictor = nn.ModuleList([nn.Linear(dims[i], dims[i + 1]) for i in range(layer_size)])

    def forward(self, raw_v_Ds):
        # each encoding
        v_Ds= []
        for i, model_drug in enumerate(self.model_drug):
            v_D = model_drug(raw_v_Ds[i])
            v_Ds.append(v_D)
        # concatenate and classify
        v_f = v_Ds[0]
        for v_D in v_Ds[1:]:
            v_f = torch.cat((v_f, v_D), 1)
...
def model_initialize(config_list):
    model = Property_Prediction(config_list)
    return model
...
class Property_Prediction:
    ...
    def __init__(self, configlist):
        ...
        for i in range(model_number):
            ...
            if drug_encoding == 'Morgan' or drug_encoding == 'ErG' or drug_encoding == 'Pubchem' or drug_encoding == 'Daylight' or drug_encoding == 'rdkit_2d_normalized' or drug_encoding == 'ESPF':
                ...
                self.model_drug.append(CNN('drug', **config))
            elif drug_encoding == 'CNN_RNN':
                self.model_drug.append(CNN_RNN('drug', **config))
            elif drug_encoding == 'Transformer':
                self.model_drug.append(transformer('drug', **config))
            elif drug_encoding == 'MPNN':
                self.model_drug.append(MPNN(config['hidden_dim_drug'], config['mpnn_depth']))
            elif drug_encoding == 'DGL_GCN':
                self.model_drug.append(DGL_GCN(in_feats=74,
            ...
            if 'decay' not in config.keys():
                config['decay'] = 0

        self.model = Classifier(self.model_drug, configlist)
        self.config = configlist
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        self.drug_encoding = drug_encoding_list
        self.result_folder = configlist[0]['result_folder']
    ...
    def test_(self, data_generator, model, repurposing_mode=False, test=False, verbose=True):
        ...
        y_label = []
        model.eval()
        for combines in zip(*data_generator):
            v_ds = []
    ...
    def train(self, train, val, test=None, verbose=True):
        ...
        lr = self.config[0]['LR']
        decay = self.config[0]['decay']

        BATCH_SIZE = self.config[0]['batch_size']
        train_epoch = self.config[0]['train_epoch']
        if 'test_every_X_epoch' in self.config[0].keys():
            test_every_X_epoch = self.config[0]['test_every_X_epoch']
        else:
        ...
        for i in range(len(self.config)):
            params = {'batch_size': BATCH_SIZE,
                      'shuffle': False,
                      'num_workers': self.config[i]['num_workers'],
                      'drop_last': False}
            if (self.drug_encoding[i] == "MPNN"):
                params['collate_fn'] = mpnn_collate_func
            elif self.drug_encoding[i] in ['DGL_GCN', 'DGL_NeuralFP', 'DGL_GIN_AttrMasking', 'DGL_GIN_ContextPred',
                                        'DGL_AttentiveFP']:
                params['collate_fn'] = dgl_collate_func

            training_generator = data.DataLoader(data_process_Multimodel_loader(train.index.values,
                                                                                     train.Label.values,
                                                                                     train, self.config),
                                                 **params)
            training_generators.append(training_generator)
            validation_generator = data.DataLoader(data_process_Multimodel_loader(val.index.values,
                                                                                           val.Label.values,
                                                                                           val, self.config),
                                                   **params)
            validation_generators.append(validation_generator)
            if test is not None:
                info = data_process_loader_Property_Prediction(test.index.values, test.Label.values, test, **self.config[i])
                ...
                testing_generator = data.DataLoader(
                    data_process_loader_Property_Prediction(test.index.values, test.Label.values, test, **self.config[i]),
                    **params_test)
        ...
        for epo in range(train_epoch):
            for combines in zip(*training_generators):
                v_ds = []
                for k in range(len(combines)):
                    v_d = combines[k][0]
                    v_d = v_d[k]
            ...
            with torch.set_grad_enabled(False):
                if self.binary:
                    ...
                    auc, auprc, f1, logits = self.test_(validation_generators, self.model)
                    ...
                    mse, r2, p_val, CI, logits = self.test_(validation_generators, self.model)
        ...
        if test is not None:
            ...
            if self.binary:
                auc, auprc, f1, logits = self.test_(testing_generators, model_max, test=True, verbose=verbose)
                ...
                mse, r2, p_val, CI, logits = self.test_(testing_generators, model_max, test=True, verbose=verbose)
